...
class CustomsDataStoreIntegrationSpec extends IntegrationTestsWithDbSpec with CustomsDataStoreService with ScalaFutures  {
  ...
  "CustomsDataStoreConnector" should {
    "call customs data store service with correct url and payload" in {
      scala.concurrent.Await.ready(customsDataStoreConnector.storeEmailAddress(dataStoreRequest), defaultTimeout)
    ...
    "return successful future with correct status when customs data store service returns good status(204)" in {
      ...
      customsDataStoreConnector.storeEmailAddress(dataStoreRequest).futureValue.status shouldBe NO_CONTENT
    }
  }
}
